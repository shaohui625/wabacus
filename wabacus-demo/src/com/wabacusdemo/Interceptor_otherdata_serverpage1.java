...
public class Interceptor_otherdata_serverpage1 extends AbsInterceptorDefaultAdapter
{
    ...
    public int beforeSave(ReportRequest rrequest,ReportBean rbean)
    {
        ...
        List<Map<String,String>> lstInsertExternalVals=rrequest.getLstInsertedExternalValues(rbean);//因为此报表为<insert/>配置了<external-values/>
        List<Map<String,String>> lstUpdateData=rrequest.getLstUpdatedData(rbean);//修改的记录集合
        List<Map<String,String>> lstUpdateExternalVals=rrequest.getLstUpdatedExternalValues(rbean);
        List<Map<String,String>> lstDeleteData=rrequest.getLstDeletedData(rbean);//删除的数据集合
        List<Map<String,String>> lstDeleteExternalVals=rrequest.getLstDeletedExternalValues(rbean);
        ...
        {//当前保存动作有修改的数据
            ...
            {//循环添加的每条记录
                ...
                String deptno_old=mUpdatevalues.get("deptno_old");//取出修改前的部门编号
                String deptname_old=mUpdatevalues.get("deptname_old");//取出修改前的部门名
                deptno=deptno==null?"":deptno.trim();
                deptname=deptname==null?"":deptname.trim();
        ...
        {//当前保存动作有删除的数据
            ...
            {//循环删除的每条记录
                ...
                String deptno=mDeletevalues.get("deptno_old");
                String deptname=mDeletevalues.get("deptname_old");
                deptno=deptno==null?"":deptno.trim();
                deptname=deptname==null?"":deptname.trim();
