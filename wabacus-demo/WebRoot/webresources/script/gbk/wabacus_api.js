 ...
 * @param invokeOnloadMethod 是否需要执行行选中的回调函数（包括配置的或框架自动生成的，比如刷新从报表等），默认为不执行，传入true则执行
 */
...
function invokeServerActionForReportData(pageid,reportid,serverClassName,shouldRefreshPage,conditionsObj,beforeCallbackMethod,afterCallbackMethod)
{
	...
	{
		wx_alert('pageid参数不能为空，必须指定页面ID');
		return;
	}
	if(reportid==null||reportid=='')
	{
		wx_alert('reportid参数不能为空，必须指定报表ID');
		return;
	}
	if(serverClassName==null||serverClassName=='')
	{
		wx_alert('serverClassName参数不能为空，必须指定要调用的服务器端类的全限定类名');
		return;
	}
