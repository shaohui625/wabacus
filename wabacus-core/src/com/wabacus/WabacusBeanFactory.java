...
public abstract class WabacusBeanFactory
{
    ...
    public static <T> T getOrCreateBean(String beanClassOrBeanId)
    {

        T bean=(T)getInstance().getBean(beanClassOrBeanId);
        if(bean==null||beanClassOrBeanId.indexOf(".")!=-1)
        {
            try
            {
                bean=(T)(ResourceUtils.loadClass(beanClassOrBeanId).newInstance());
