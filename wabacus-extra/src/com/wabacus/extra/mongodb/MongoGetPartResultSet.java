...
public class MongoGetPartResultSet extends MongodbGetAllResultSet {
	private static final Log LOG = LogFactory
			.getLog(MongoGetPartResultSet.class);
	 ...
	 //       sqlKernel=ReportAssistant.getInstance().parseRuntimeSqlAndCondition(rrequest,rbean,datasetbean,sqlKernel,lstConditions,lstConditionsTypes);
	   ...
	   //     sqlCount=ListReportAssistant.getInstance().addColFilterConditionToSql(rrequest,rbean,datasetbean,sqlCount);
		...
		if (rbean.getInterceptor() != null) {
			Object obj = rbean.getInterceptor().beforeLoadData(rrequest, rbean,
					reportTypeObj, sqlCount);
			if (obj == null)
				return -1;
			if (obj instanceof List) {
				...
				throw new WabacusRuntimeException("执行报表" + rbean.getPath()
						+ "的加载数据拦截器失败，返回的数据类型" + obj.getClass().getName()
						+ "不合法");
			}

		}
		Number count = (Number) MongoExprContext.initExtraVarsIf(rrequest).get(
				"resultCount");
		if (count != null) {
		...
		try {
			if (StringUtils.isNotBlank(sqlCount)) {
				MongoExprContext ctx = eval(rrequest, rbean, datasetbean,sqlCount);
			...
			throw new WabacusRuntimeException("从数据库取数据时执行SQL：" + sqlCount
					+ "失败", e);
		} finally {
