...
public final class SystemHelper {
	 ...
	 *            - SPI接口类
	 * @param classLoader
	 *            - 使用的类加载器,可为null(此时先采用Thread getContextClassLoader
	 *            ,如还为空则采用serviceClass的ClassLoader)
	 * @return 加载以指定的SPI实现
	 */
	public static <T> T loadService(Class<T> serviceClass,
		...
		if (!providers.hasNext()) {
			throw new IllegalAccessError(
					"请确保类路径下存在指定文本文件且其只有此服务实现类文件全名一行! META-INF/services/"
	...
	public static <T> T loadServiceIf(Class<T> serviceClass,
			ClassLoader classLoader) {
		Iterator<T> providers = loadServies(serviceClass, classLoader);
		if (providers.hasNext()) {
			T serviceImpl = (T) providers.next();
			if (providers.hasNext()) {
				LOG.warn("存在多个服务只取第一个!以下服务实现不会采用:{}",
						Lists.newArrayList(providers));
			}
			return serviceImpl;
		} else {
			LOG.info("在请类路径META-INF/services/{}下指定服务实现类",
					serviceClass.getName());
		}
		return null;
	}
	...
	private static <T> Iterator<T> loadServies(Class<T> serviceClass,
			ClassLoader classLoader) {
		if (null == classLoader) {
		...
		if (isJavaVersionAtLeast(1.6f)) {
			...
			providers = ServiceLoader.load(serviceClass, classLoader)
					.iterator();
		} else {
	...
	public static InputStream getResourceAsStream(String name, Class refClass) {
		final InputStream in = refClass != null ? refClass
				.getResourceAsStream(name) : null;
		return in != null ? in : getClassLoader(refClass).getResourceAsStream(
				name);
	}
	...
	public static Throwable getRootCause(Throwable ex) {
		...
		return null == rootCause ? ex : rootCause;
	}
	...
	private static final Pattern ERR_PATTERN = Pattern.compile("(Expression: false)|(^el exception:)|([.] Expression:.*)");

	public static String friendlyMessage(String msg) {
		final String message = msg != null ? ERR_PATTERN.matcher(msg).replaceAll("") : null;
		return message;
	}
}
